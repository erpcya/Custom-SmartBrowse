...
public class VBrowser extends Browser implements ActionListener,
	...
	protected void prepareTable(Info_Column[] layout, String from,
		...
		for (int i = 0; i < layout.length; i++) {
			...
			detail.addBrowseField(i, m_Browse.getField(layout[i].getColHeader()));
	...
	protected void saveSelection() {
		 ...
		 * Setting a comment for not reload panel detail
		 */
	...
	private void bOkActionPerformed(java.awt.event.ActionEvent evt) {
		...
		if (m_Browse.getAD_Process_ID() > 0 && getSelectedKeys() != null)
		{
			...
			/**
			 * @author Carlos Parada 
			 * Validating parameters of process
			 */
			// call process 
			if (parameterPanel.saveParameters()){
				...
				ADialog.info(Env.getWindowNo(m_frame), m_frame, pi.getSummary());
				/**
				 * @author Carlos Parada
				 * Clear Selection
				 */
				m_ok= false;
				p_multiSelection = false;
				saveSelection();
			}
	...
	/** Arraylist Amounts Collector */
	private ArrayList<BigDecimal> m_Amts = new ArrayList<BigDecimal>();
	...
	public static void main(String args[]) {
		...
		MBrowse browse = new MBrowse(m_ctx, 1000000, null);
	...
	/**
	 * @author Carlos Parada 18/02/2013, 14:42:33
	 * @return boolean
	 */
	 ...
	 * Class to control Event Selected
	 * 
	 */
	/*@SuppressWarnings("serial")
	class BrowserTable extends MiniTable 
	{
		...
		public boolean editCellAt(int row, int column, EventObject e) {
			// TODO Auto-generated method stub			
			if (getFieldKey().getName().equals(getColumn(column).getHeaderValue()))
	...
	/**
	 * @author <a href="mailto:carlosaparadam@gmail.com">Carlos Parada</a> May 21, 2013, 11:32:17 AM
	 * @return void
	 * Create Arraylist with Columns 
	 */
	protected void create_Amts(int row,boolean add)
	{
		int column=0;
		if (row==-1)
		{
		...
		else
		{
			for (int col = 0; col < p_layout.length; col++)
			{
				...
				{
					if (add)
						m_Amts.set(column,m_Amts.get(column).add((BigDecimal)detail.getModel().getValueAt(row, col)));
					else
						m_Amts.set(column,m_Amts.get(column).subtract((BigDecimal)detail.getModel().getValueAt(row, col)));
					column++;
				}
				else if (c == Double.class)
				{
					if (add)
						m_Amts.set(column,m_Amts.get(column).add(new BigDecimal((Double)detail.getModel().getValueAt(row, col))));
					else
						m_Amts.set(column,m_Amts.get(column).subtract(new BigDecimal((Double)detail.getModel().getValueAt(row, col))));
					column++;
				}		
			}
		}
	...
	/**
	 * 
	 * @author <a href="mailto:carlosaparadam@gmail.com">Carlos Parada</a> May 22, 2013, 9:17:08 AM
	 * @return void
	 * set Status Bar Amts Selected
	 */
	protected void updateSB()
	{
		String amt="";
		
		for (BigDecimal amount :m_Amts)
			amt+=(amt.equals("")?amount.setScale(2, BigDecimal.ROUND_HALF_UP).toString() : " | " + amount.setScale(2, BigDecimal.ROUND_HALF_UP).toString());
		
		setStatusLine(Msg.translate(Env.getCtx(),"Selected") + " " + amt, false);
	}
